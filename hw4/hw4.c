...
binary_semaphore doorlock; // to protect the
...
inline void guard_check_room()
{
  ...
  // guard is waiting outside the room
  if(num_students > 0 && num_students < capacity){
    guard_state = -1;
    //printf("\tguard waiting to enter the room with %2d students...\n", num_students);
    //millisleep(ms);
  ...
  if(num_students >= capacity){
    ...
    printf("\tguard is clearing out room with %2d students...\n", num_students);
...
//semWaitB(doorlock);
if(guard_state > 0){
  // Guard is in the room, No entrance
}

else{
  // lock and incrament students in the room
  semWaitB(&mutex);
  num_students++;
  semSignalB(&mutex);
  ...
  // lock and decrement students in the room
  semWaitB(&mutex);
  num_students--;
  //semSignalB(&mutex);

  // last student leave room with guard in it
  if(num_students == 0 && guard_state == 1){
    printf("LAST student %2d left room with guard in it\n", id);
  }
  else{
    printf("student %2ld left room\n", id);
  }
  // PRINT student number and left room
